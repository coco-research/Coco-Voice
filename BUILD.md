...
This guide covers how to set up the development environment and build Coco Voice from source across different platforms.
...
```bash
git clone git@github.com:coco-research/Coco-Voice.git
cd Coco Voice
```
...
```
Bundling Coco Voice_*_amd64.AppImage
...
~/.cache/tauri/linuxdeploy-x86_64.AppImage --appimage-extract-and-run \
  --appdir Coco Voice.AppDir --plugin gtk --output appimage
```
...
it builds from a short root such as `D:\a\Coco Voice`.)
...
# Rust projects' build output, not just Coco Voice):
...
If the build compiles all the way to `Built application at: ...\CocoVoice.exe` and
then fails with:

```
Signing C:\...\CocoVoice.exe with a custom signing command

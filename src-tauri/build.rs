fn main() {
    ...
    // next to CocoVoice.exe so the app loads our baseline build instead of statically
...
///
/// Coco Voice's native stack links the VC++ runtime dynamically (/MD). Shipping the
...
/// DLL families Coco Voice imports and no-ops when the env var is unset.
...
/// `CocoVoice.exe` (Windows resolves DLLs from the executable's directory).
///

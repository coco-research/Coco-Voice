...
pub fn get_icon_path(theme: AppTheme, state: TrayIconState) -> &'static str {
    match (theme, state) {
        ...
        (AppTheme::Colored, TrayIconState::Idle) => "resources/coco-voice.png",
...
fn version_label() -> String {
    if cfg!(debug_assertions) {
        format!("Coco Voice v{} (Dev)", env!("CARGO_PKG_VERSION"))
    } else {
        format!("Coco Voice v{}", env!("CARGO_PKG_VERSION"))
    }
}
...
mod tests {
    ...
    fn build_entry(transcription: &str, post_processed: Option<&str>) -> HistoryEntry {
        HistoryEntry {
            id: 1,
            file_name: "coco-voice-1.wav".to_string(),
            timestamp: 0,
            saved: false,

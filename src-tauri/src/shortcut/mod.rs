    ...
    match user_settings.keyboard_implementation {
        ...
        KeyboardImplementation::CocoVoiceKeys => {
            if let Err(e) = coco_keys::init_shortcuts(app) {
                ...
                // Update settings to persist the fallback so we don't retry CocoVoiceKeys on next launch
...
pub fn register_cancel_shortcut(app: &AppHandle) {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::CocoVoiceKeys => coco_keys::register_cancel_shortcut(app),
    }
}
...
pub fn unregister_cancel_shortcut(app: &AppHandle) {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::CocoVoiceKeys => coco_keys::unregister_cancel_shortcut(app),
    }
}
...
pub fn register_shortcut(app: &AppHandle, binding: ShortcutBinding) -> Result<(), String> {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::CocoVoiceKeys => coco_keys::register_shortcut(app, binding),
    }
}
...
pub fn unregister_shortcut(app: &AppHandle, binding: ShortcutBinding) -> Result<(), String> {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::CocoVoiceKeys => coco_keys::unregister_shortcut(app, binding),
    }
}
...
pub fn get_keyboard_implementation(app: AppHandle) -> String {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::CocoVoiceKeys => "coco_keys".to_string(),
    }
}
...
fn validate_shortcut_for_implementation(
    ...
    match implementation {
        ...
        KeyboardImplementation::CocoVoiceKeys => coco_keys::validate_shortcut(raw),
    }
}
...
fn parse_keyboard_implementation(s: &str) -> KeyboardImplementation {
    match s {
        ...
        // "handy_keys" is a legacy alias for backward compat
        "coco_keys" | "handy_keys" => KeyboardImplementation::CocoVoiceKeys,
        other => {
            warn!(
...
/// Initialize CocoVoiceKeys if not already initialized, with rollback on failure
fn initialize_handy_keys_with_rollback(app: &AppHandle) -> Result<bool, String> {
    if app.try_state::<coco_keys::CocoVoiceKeysState>().is_some() {
        return Ok(false); // Already initialized, caller should continue
    }

    if let Err(e) = coco_keys::init_shortcuts(app) {
        error!("Failed to initialize CocoVoiceKeys: {}", e);
        // Rollback to Tauri
        let mut settings = settings::get_settings(app);
        settings.keyboard_implementation = KeyboardImplementation::Tauri;
        settings::write_settings(app, settings);
        tauri_impl::init_shortcuts(app);
        return Err(format!(
            "Failed to initialize CocoVoiceKeys: {}. Reverted to Tauri.",
            e
        ));
    }
...
#[tauri::command]
#[specta::specta]
pub fn update_corrections(
    app: AppHandle,
    corrections: Vec<settings::CorrectionPair>,
) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);

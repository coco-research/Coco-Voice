...
interface CocoVoiceKeysShortcutInputProps {
...
export const CocoVoiceKeysShortcutInput: React.FC<CocoVoiceKeysShortcutInputProps> = ({
  ...
  // Set up event listener for coco-keys events
  useEffect(() => {
    if (!isRecording) return;
    ...
    const setupListener = async () => {
      ...
      const unlisten = await listen<CocoVoiceKeysEvent>(
        "coco-keys-event",
        async (event) => {
          if (cleanup) return;
